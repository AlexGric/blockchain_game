...
contract('Memory Token', (accounts) => {
  ...
  describe('token destribution', async () => {
    let result

    it('mints token', async () => {
     await token.mint(accounts[0], 'https://www.nft-coolection.com/nft');
     ...
     assert.equal(result.toString(), accounts[0].toString(), 'owner of is correct');
     result = await token.tokenOfOwnerByIndex(accounts[0], 0)
      ...
      assert.equal(tokenURI, 'https://www.nft-coolection.com/nft')
    })
